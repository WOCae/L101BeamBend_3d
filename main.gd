...
func _on_button_pressed():
	...
	print($Line2D.get_point_position(1).x * 3/coef)
	#$Label_L2.position.x = 

	#----3D表示形状
	$"../CSGBox3D".size.x = float($"LineEdit_L".text)
	$"../CSGBox3D".size.y = float($"LineEdit_h".text)
	$"../CSGBox3D".size.z = float($"LineEdit_b".text)
